...
public class Spinner : Control {
    ...
    [Browsable(true)]
    [Category("Appearance")]
    public double NodeResizeRatio { get; set; } = 1.0f;
    ...
    protected override void OnPaint(PaintEventArgs e) {
        ...
        for (int i = next, a = 0; i < next + NodeCount; i++, a++) {
            ...
            double currRad = NodeRadius + a * NodeResizeRatio;
            PointF c1 = new PointF((float)(c1X - currRad), (float)(c1Y - currRad));

            using (Brush brush = new SolidBrush(NodeFillColor)) {
                e.Graphics.FillEllipse(brush, (float)c1.X, (float)c1.Y, (float)(2 * currRad), (float)(2 * currRad));
            }

            using (Pen pen = new Pen(Color.White, NodeBorderSize)) {
                e.Graphics.DrawEllipse(pen, (float)c1.X, (float)c1.Y, (float)(2 * currRad), (float)(2 * currRad));
            }
        }
    }
