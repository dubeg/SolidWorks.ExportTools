...
public class ModelToPdfCommand : CommandBase<AppSettings> {
    private FrmModelToPdfExporterMulti _multiExportForm;

    public ModelToPdfCommand(AddinUiManager uiMgr, ISwApplication swApp, AppSettings appSettings, AddIn addin) 
    ...
    public void ShowMultiExportForm() {
        if (_multiExportForm == null || _multiExportForm.IsDisposed) {
            UiManager.InitAndShowForm(() => _multiExportForm = new FrmModelToPdfExporterMulti(this));
        }
        else {
